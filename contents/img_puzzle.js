...
function img_pzl(options) {
  ...
  try {
    if (typeof options !== "object")
      throw (
        "An object must be passed to the function! Now it is: " + typeof options
      )
  } catch (err) {
  ...
  try {
    if (typeof images !== "string" && !Array.isArray(images))
      throw (
        "images expecting to be a string or an array, " +
        typeof images +
        " given"
      )
    ...
    if (typeof transition !== "number" && typeof transition !== "undefined")
      throw (
        "transition expecting to be a number, " + typeof transition + " given"
      )
    ...
    if (
      typeof win_function !== "function" &&
      typeof win_function !== "undefined"
    )
      throw (
        "img_pzl.gameOver expecting to be a function, " +
        typeof win_function +
        " given"
      )
    ...
    if (typeof difficulty !== "string")
      throw (
        "difficulty expecting to be a string, " + typeof difficulty + " given"
      )
    ...
    if (typeof on_shuffle !== "function" && typeof on_shuffle !== "undefined")
      throw (
        "onShuffle expecting to be a function, " + typeof on_shuffle + " given"
      )

    if (
      typeof on_shuffle_end !== "function" &&
      typeof on_shuffle_end !== "undefined"
    )
      throw (
        "onShuffleEnd expecting to be a function, " +
        typeof on_shuffle_end +
        " given"
      )

    if (
      typeof on_hint_swap !== "boolean" &&
      typeof on_hint_swap !== "undefined"
    )
      throw (
        "hintSwap expecting to be a boolean, " + typeof on_hint_swap + " given"
      )

    // checking if the main holder div's height and width has been set
    if (
      document.querySelector(div_holder).offsetHeight == 0 ||
      document.querySelector(div_holder).offsetWidth == 0
    )
      throw (
        "The main holder's ( " +
        div_holder +
        " ) width or height has been set to 0. It cannot be seen"
      )
  } catch (err) {
    console.error("img_pzl : " + err + "!")
    return
  ...
  img.onload = function () {
    ...
    if (document.querySelectorAll(div_holder + " ._game_output .bg-elem")) {
      let element = document.querySelectorAll(
        div_holder + " ._game_output .bg-elem"
      )
    ...
    for (let i = 0; i <= elem_piece; i++) {
      ...
      game_holder.innerHTML +=
        "<div data-sequence=" + seq_array[i] + " class='bg-elem'></div>\n"
    }

    // storing all elements in this constant
    const element = document.querySelectorAll(
      div_holder + ` .${holder_class} .bg-elem`
    )
    ...
    for (let i = 0; i < element.length; i++) {
      ...
      if (column === columns) {
        // if last column, go to the next row
        column = 0
        row++
      }
    ...
    function event_function(event) {
      ...
      if (event.type === "touchend" || event.type === "mouseup") {
        ...
        for (let i = 0; i < element.length; i++) {
          ...
          if (
            mouse_pos[0] > element[i].offsetLeft &&
            mouse_pos[0] < element[i].offsetLeft + element_width &&
            mouse_pos[1] > element[i].offsetTop &&
            mouse_pos[1] < element[i].offsetTop + element_height
          ) {
    ...
    function swap_elem(elem1, elem2) {
      let num1 = parseInt(elem1),
        num2 = parseInt(elem2)

      let temp = element[num1].getAttribute("data-sequence")
      element[num1].setAttribute(
        "data-sequence",
        element[num2].getAttribute("data-sequence")
      )
    ...
    img_pzl.hint = function () {
      ...
      while (help === false) {
        ...
        if (seq_array[random] !== number) {
          for (let i = 0; i < element.length; i++) {
            if (
              parseInt(element[i].getAttribute("data-sequence")) ===
              seq_array[random]
            ) {
              if (on_hint_swap) {
                swap_elem(i, random)
          ...
          if (arrayEquals(seq_array, data_sequence)) {
            ...
            setTimeout(function () {
              ...
              chrome.storage.local.get(["qrackedSolved"], (res) => {
                const currQrackedSolve = res["qrackedSolved"]
                const newQrackedSolve = currQrackedSolve + 1
                chrome.storage.local.set({ qrackedSolved: newQrackedSolve })
              })
